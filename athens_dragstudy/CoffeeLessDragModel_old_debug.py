...
def platedrag(plate_len, plate_thi, plate_wid, plate_n, ang, vel, mu, rho, q):
    ...
    # front face things
    plate_cf = plate_cf + (
        0.03 * (plate_thi / plate_thi) / (plate_Re ** (1 / 5))
    ...
    pitch[abs(pitch) > 90] = (abs(pitch[abs(pitch) > 90]) - 90) * signs0[
        abs(pitch) > 90
    ]
    signs = np.sign(pitch) + (pitch == 0) * 1  # get sign, watches for the zero case
    pitch = abs(pitch)
...
def ellipticaldrag(ell_chord, ell_len, ell_dia, ell_n, ang, vel, mu, rho):
    ...
    keep = (
        ell_Re <= 10**6
    )  # if above transtion Reynolds number, then we make it turbulent
    ...
    ell_cdb = (
        ell_cd * (np.pi * (ell_dia / 2) ** 2) * ell_dia**2
    )  # we have to do a conversion here as area = ell_dia^2 for this drag coefficient, not the typical area = pi*r^2
    ...
    ell_cd = (ell_cd * np.pi * (ell_dia / 2) ** 2 + np.pi * ell_cf) / (
        np.pi * (ell_dia / 2) ** 2
    )  # (3-11)
    # Calculate reference area
    warea = (
...
def run_full(DataName, ParaName, include_wing, create_plot, debug, stl_output):
    ...
    # hack for directly reading the dict instead of writing to file
    if isinstance(DataName, dict) and isinstance(ParaName, dict):
        # use the dictionaries directly to limit file IO
        # print("[DRAG MODEL] --- operating directly on data and parameter dictionaries")
        spatial = DataName
        parameter = ParaName
    ...
    for o in np.arange(1, 4, 1):  # change second digit to 4  to get all 3 directions
        ...
        for q in spatial:
            ...
            #     ecolor = [255 / 255, 255 / 255, 0]
                ...
                mesh = trimesh.creation.cylinder(
                    diafuse / 2,
                    height=float(parameter[q]["TUBE_LENGTH"]),
                    transform=mesh2cad,
                    center_mass=spatial[q]["CG"],
                    sections=10,
                )
                ...
                cd, cl, cf, warea = ellipticaldrag(
                    (float(parameter[q]["TUBE_LENGTH"]) + diafuse) / 1000,
                    (float(parameter[q]["TUBE_LENGTH"]) + diafuse) / 1000,
                    diafuse / 1000,
                    cap_norm,
                    ang,
                ...
                mesh2cad = [
                    [1, 0, 0, -float(parameter[q]["CHORD"]) / 2],
                    [0, 1, 0, float(parameter[q]["SPAN"]) / 4],
                    [0, 0, 1, 0],
                    [0, 0, 0, 1],
                ]
                mesh = trimesh.creation.box(
        ...
        interf = dict(
            sorted(spatial.items(), key=lambda item: item[1]["GCG"][0], reverse=True)
        )
    ...
    if debug == False:
        ...
        if stl_output and create_plot:
            return (drags, center, Con_Mesh, all_figs, spatial, parameter, prop)
...
if __name__ == "__main__":
    ...
    DB = True
    if DB == True:
        drags, center, spatial, parameter, prop = run_full(
            DataName="designData.json",
            ParaName="designParameters.json",
            include_wing=True,
            create_plot=True,
            debug=True,
            stl_output=True,
        ...
        # for k in spatial:
        #     #print(k, type(v))
        #     if isinstance(spatial[k], np.ndarray):
        #         x = spatial[k].tolist()
        #         spatial[k] = None
        #         spatial[k] = x
        #     elif isinstance(spatial[k], dict):
        #         for kk in spatial[k]:
        #             #print(kk, type(vv))
        #             if isinstance(spatial[k][kk], np.ndarray):
        #                 xx = spatial[k][kk].tolist()
        #                 spatial[k][kk] = None
        #                 spatial[k][kk] = xx
        # for k in spatial:
        #     print(type(spatial[k]))
        # spatial_fin = {k:v.tolist() for k,v in spatial.items()}
        print(
            f"debug output types: spatial: {type(spatial)} \
        parameter: {type(parameter)} prop: {type(prop)}"
        )

        for k in spatial:
            if "Drag" in spatial[k]:
                print(k, spatial[k]["Drag"].shape)

        # with open("./spatial_fin.json", "w") as s:
        #     s.write(json.dumps(spatial, indent=2))

        # with open("./parameter_fin.json", "w") as s:
        #     s.write(json.dumps(parameter, indent=2))

        # with open("./prop_fin.json", "w") as s:
        #     s.write(json.dumps(prop, indent=2))

    else:
        drags, center = run_full(
            DataName="designData.json",
            ParaName="designParameters.json",
            include_wing=True,
            create_plot=False,

...
UAM_DATA = os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.dirname(__file__), "data/uam"))
UAV_DATA = os.path.abspath(os.path.join(os.path.dirname(__file__), os.path.dirname(__file__), "data/uav"))


# TODO: if you want a new vehicle to study it must go here
UAM_VEHICLES = ["spade", "tbar", "tbar_full", "tbar_double", "tbar_single"]
...
class DragRunner:
    def __init__(self, vehicle: str, num_runs: int, study_params: list, run_baseline: bool = False):
    ...
    def run_dragmodel(self):
        ...
        if self.run_baseline:  # run the analysis with the original parameters
            if not os.path.exists(os.path.join(self.BASE_PATH, 'baseline')):
                ...
                print(f"baseline results for {self.vehicle} ({self.vehicle_type}) already exist, exiting")
                return
            ...
            with open(os.path.join(self.BASE_PATH, 'baseline', 'drag_center.txt'), 'w') as f:
                f.write(str(drags) + "\n")
                f.write(str(center))
            ...
            for drxn, plot in list(zip(['x', 'y', 'z'], plots)):
                plot.savefig(os.path.join(self.BASE_PATH, 'baseline', f'drag_plot_{drxn}.png'))
        ...
        for i in range(self.num_runs):
            ...
            print(f"[TIME TAKEN] {e-s}s for iteration {i+1}/{self.num_runs} to update params and store results")
        ...
        with open(os.path.join(self.BASE_PATH, 'results', self.vehicle + "_" + time.strftime("%Y%m%d-%H%M%S") + ".csv"), 'w') as f:
            writer = csv.writer(f)
    ...
    def update_drag_input_params(self, scale=20):
        ...
        if "length" in self.study_params:
            ...
            for structure, properties in self.paramdict.items():
                #print(structure)
                cadpart = "fuse_cyl_ported.prt" if self.vehicle_type == "uam" else "para_tube.prt"
                if "LENGTH" in properties.keys() and properties['CADPART'] == cadpart:  # only change length for now
        ...
        if "wing" in self.study_params:
            ...
            for structure, properties in self.paramdict.items():
                ...
                cadpart = "naca_sym_wing_taper.prt" if self.vehicle_type == "uam" else "uav_wing.prt"
                if "NACA_PROFILE" in properties.keys() and properties['CADPART'] == cadpart:
                    ...
                    for idx, orig_val in enumerate([orig_c1, orig_c2, orig_span]):
                        ...
                        if structure in changed:
                            print(f"updating {structure} params from {orig_val} to {new_val}")
                            changed[structure][names[idx]] = (orig_val, new_val)
                        else:
                            print(f"init {structure} params with {orig_val} and {new_val}")
                            changed[structure] = {names[idx]: (orig_val, new_val)}
...
def run():
    ...
    #                     choices=['uav', 'uam'])
    parser.add_argument('--vehicle', type=str, metavar='design name',
                        help="the name of the vehicle of interest")
    parser.add_argument('--fit', action='store_true',
                        help="fit the specified vehicle to")
    parser.add_argument('--baseline', action='store_true',
                        help="run the drag model for the original vehicle parameters")
    parser.add_argument('--rand-length', action='store_true',
                        help="randomly change connector lengths in the design by a small amount")
    parser.add_argument('--rand-wing', action='store_true',
                        help="randomly change connector lengths in the design by a small amount")
    parser.add_argument('--rand-prop', action='store_true',
                        help="randomly change connector lengths in the design by a small amount")
    parser.add_argument('--runs', type=int,
                        help="the number of drag runs to complete")
